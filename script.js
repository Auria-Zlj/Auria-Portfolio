...
window.addEventListener('load', () => {
    ...
    // For testing: immediately show cards and position them
    setTimeout(() => {
        console.log('Showing cards for testing...');
        nodes.forEach(node => {
            const card = document.getElementById(`card-${node.id}`);
            if (card) {
                console.log(`Found card ${node.id}:`, card);
                
                // Clear only position styles, keep visibility
                card.style.removeProperty('left');
                card.style.removeProperty('top');
                card.style.removeProperty('animation');
                
                // Override the animation and show the card immediately
                card.style.animation = 'none';
                card.style.transform = 'translate(-50%, -50%) scale(1)';
                card.style.opacity = '1';
                
                console.log(`Card ${node.id} made visible, current styles:`, {
                ...
                console.error(`Card ${node.id} not found!`);
            }
        });
        calculatePositions();
    }, 2000); // Show cards after 2 seconds for testing
